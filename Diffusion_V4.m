...
Nx = 51; %(This will differ to observe behavior)
Ny = 51; %(This will differ to observe behavior)
...
for n = -1 : Nt
    ...
    for i = 2:Nx-1 
        for j = 2:Ny-1
            Uc(j,i) = Un(j, i) + ((dt/(dx.^2)).*((Un(j, i+1) + Un(j+1, i) - 4*Un(j, i) + Un(j, i-1) + Un(j-1, i)))); % explicit
            Uc(j,i) = Un(j, i) + ((dt/(dx.^2)).*((Uc(j, i+1) + Uc(j+1, i) - 4*Uc(j, i) + Uc(j, i-1) + Uc(j-1, i)))); % implicit 
